    ...
    <div>
          ...
          {plans.map(plan => {
            ...
            return (
              <div
                key={plan.name}
                className={cn(
                  'relative bg-white dark:bg-white/5 border rounded-xl p-6 transition-all duration-300',
                  isCurrent
              ...
              >
                ...
                {plan.highlighted && !isCurrent && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="inline-flex items-center gap-1 px-3 py-1 bg-slate-900 text-white dark:bg-white dark:text-black text-[10px] font-bold uppercase tracking-wider">
                ...
                <div className="mb-4 px-3 py-2 bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] rounded-lg">
                  <p className="text-[11px] font-semibold text-emerald-400">{extras.roi}</p>
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    Min. investment: <span className="font-semibold text-slate-900 dark:text-white">{extras.minInvestment}</span>
                  </p>
                </div>
                  ...
                  {plan.features.map(feature => (
                    <li key={feature} className="flex items-start gap-2 text-sm">
                      <span className={cn(
                        ...
                        isCurrent ? 'bg-slate-900 text-white dark:bg-white dark:text-black' : 'bg-slate-100 dark:bg-white/10 text-slate-900 dark:text-white'
                      )}>
                        <Check size={10} strokeWidth={3} />
                      </span>
                      <span className="text-slate-900 dark:text-white leading-snug text-xs">{feature}</span>
                    </li>
                  ))}
                </ul>
                ...
                {isCurrent ? (
                  ...
                  <button
                    ...
                    className="w-full border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white hover:border-slate-300 dark:hover:border-white/30 hover:bg-slate-50 dark:hover:bg-white/5 font-bold py-2.5 text-sm rounded-lg transition-colors flex items-center justify-center gap-2"
                  >
      ...
      {showModal && selectedPlanData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-[#111] border border-slate-200 dark:border-white/[0.08] rounded-xl w-full max-w-sm p-6 shadow-2xl">
            <button onClick={closeModal} className="absolute top-4 right-4 w-7 h-7 flex items-center justify-center text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
              <X size={16} />
            </button>

            <h3 className="text-base font-bold text-slate-900 dark:text-white mb-1">Switch to {selectedPlanData.name}?</h3>
            <p className="text-xs text-slate-400 mb-4">
              Moving from <span className="font-semibold text-slate-900 dark:text-white">{currentPlan}</span> to{' '}
              <span className="font-semibold text-slate-900 dark:text-white">{selectedPlanData.name}</span>.
            </p>

            <div className="p-4 bg-white/[0.04] border border-slate-200 dark:border-white/[0.08] rounded-lg mb-5">
              <div className="flex items-baseline gap-1 mb-1">
                {selectedPlanData.price === 0 ? (
                  <span className="text-xl font-bold text-slate-900 dark:text-white">Free</span>
                ) : (
                  <>
                    <span className="text-lg font-bold text-slate-900 dark:text-white">${selectedPlanData.price}</span>
              ...
              <p className="text-xs text-slate-400 mt-1">
                Min. investment: <span className="font-semibold text-slate-900 dark:text-white">{planExtras[selectedPlanData.name].minInvestment}</span>
              </p>
            </div>

            {saveError && (
              <p className="text-xs text-brand-primary mb-3">{saveError}</p>
            )}
            ...
            <div className="flex gap-3">
              <button
                onClick={closeModal}
                disabled={saving}
                className="flex-1 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white font-semibold py-2.5 text-sm rounded-lg hover:border-slate-300 dark:hover:border-white/30 transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={saving}
                className="flex-1 bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 disabled:opacity-60 disabled:cursor-not-allowed text-white dark:text-black font-bold py-2.5 text-sm rounded-lg transition-colors flex items-center justify-center gap-2"
              >
