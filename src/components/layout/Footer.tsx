...
const footerLinks = {
  ...
  Support: ['Help Center', 'Community', 'Status', 'Security', 'Compliance'],
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 mb-12">
            ...
            <div className="flex items-center gap-3">
              {socials.map(({ label, svg }) => (
                <span
                  key={label}
                  aria-label={label}
                  className="w-9 h-9 rounded-lg bg-light-border dark:bg-dark-card border border-light-border dark:border-dark-border flex items-center justify-center text-slate-500 dark:text-slate-400"
                >
                  {svg}
                </span>
              ))}
          ...
          {Object.entries(footerLinks).map(([group, links]) => (
            <div key={group}>
              <h4 className="text-xs font-semibold text-dark-base dark:text-light-base uppercase tracking-wider mb-4">
                {group}
              </h4>
